...
private:
    StopID id; // Stop id
    Name name; // Stop name
    Coord coord; // Stop location
    std::vector<StopNode*> nextStops; // Following stops
    RegionNode* region; // smallest region, where this stop can be found

    std::vector<RouteNode*> routes; // Routes that go through this stop
...
public:
    ...
    bool addRoute(RouteNode* route);
    std::vector<RouteNode *> getRoutes();
